    ...
    <main data-screen-label="Dashboard" style={{background:"var(--cream-2)", minHeight:"calc(100vh - 64px)"}}>
      <div style={{display:"grid", gridTemplateColumns:"260px 1fr", minHeight:"calc(100vh - 64px)"}}>
        ...
        <section style={{padding:"40px 48px", overflowY:"auto"}}>
          {section === "overview"  && <OverviewSection  setPage={setPage} setSection={setSection} onBook={onBook}/>}
          {section === "sessions"  && <SessionsSection  />}
          {section === "messages"  && <MessagesSection  />}
          {section === "earnings"  && <EarningsSection  />}
          {section === "skills"    && <SkillsSection    onBook={onBook} setPage={setPage}/>}
...
// ── Overview ─────────────────────────────────────────────────
function OverviewSection({ setPage, setSection, onBook }) {
  return (
    <>
  ...
  return (
    <div style={{height:"calc(100vh - 180px)", display:"grid", gridTemplateColumns:"280px 1fr", gap:0, border:"1px solid var(--line)", borderRadius:16, overflow:"hidden", background:"var(--paper)"}}>
      ...
      <div style={{borderRight:"1px solid var(--line)", overflow:"auto"}}>
        ...
        {threads.map((t, i) => (
          <div key={i} onClick={() => setActive(i)} style={{
            padding:"16px 20px", cursor:"pointer", display:"flex", gap:12, alignItems:"center",
            ...
            <div style={{flex:1, minWidth:0}}>
              <div className="row between">
                ...
                <span style={{fontSize:11, color:"var(--ink-4)"}}>{t.time}</span>
              </div>
      ...
      <div style={{display:"flex", flexDirection:"column"}}>
        <div style={{padding:"16px 24px", borderBottom:"1px solid var(--line)", display:"flex", gap:12, alignItems:"center"}}>
          <Avatar initials={thread.initials} size="sm" color={thread.color}/>
          <strong style={{fontSize:15}}>{thread.name}</strong>
          <button className="btn btn-sm btn-ghost" style={{marginLeft:"auto"}} onClick={() => window.showToast(`Video call with ${thread.name} starting…`, "📹")}>Video call</button>
        </div>
        <div style={{flex:1, padding:"24px", overflowY:"auto", display:"flex", flexDirection:"column", gap:12}}>
          {thread.msgs.map((m, i) => (
            <div key={i} style={{display:"flex", justifyContent: m.from==="me" ? "flex-end" : "flex-start"}}>
              <div style={{
                maxWidth:"72%", padding:"10px 16px", borderRadius: m.from==="me" ? "18px 18px 4px 18px" : "18px 18px 18px 4px",
      ...
      <div style={{display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:16, marginBottom:32}}>
        {[
          ...
          <div key={i} className="card" style={{padding:24}}>
            ...
            <div className="display" style={{fontSize:44, marginTop:8}}>{c.val}</div>
...
// ── My Skills ────────────────────────────────────────────────
function SkillsSection({ onBook, setPage }) {
